# Superstore Sales Analysis Dashboard (Power BI)

##  Project Overview
This project presents an interactive Power BI dashboard analyzing sales performance across customers, products, and regions. It highlights key business insights, trends, and opportunities to support data-driven decision-making.

##  Dashboard Preview
### Executive Overview
![Executive Overview](https://github.com/user-attachments/assets/284b95ef-83cb-4975-8745-3f07ca0aa3ea)
### Customer Analysis
![Customer Analysis](https://github.com/user-attachments/assets/7620441e-4810-4750-9b89-1f19dca6b5ae)
### Product Analysis
![Product Analysis](https://github.com/user-attachments/assets/ad9f12a0-dd03-467b-ba49-878915b39353)
### Geography Analysis
![Geography Analysis](https://github.com/user-attachments/assets/170bad97-dfb7-4e88-b92d-f8052945368f)

##  Dataset
The dataset includes order-level data covering customers, products, sales, and geographic regions.

##  Key Features
- Interactive dashboard with slicers (Year, Quarter)
- Customer segmentation and distribution analysis
- Product performance analysis (Category & Sub-category)
- Geographic analysis with map visualization
- Drill-through functionality for customer-level insights

##  Key Insights
- Sales declined by 4% in 2016 before recovering in 2017
- The Consumer segment is the largest contributor to overall sales
- Phones and Chairs contribute 30% of total sales
- The West region leads, with strong performance driven by California

##  Tools Used
- Power BI (Data Visualization)
- DAX (Data Analysis Expressions)
- Power Query (Data Transformation)

##  Project Files
- Power BI file (.pbix)
- Dashboard export (.pdf)

##  How to Use
- Use slicers to filter by Year and Quarter  
- Navigate between pages using the top menu  
- Right-click on a customer to access detailed drill-through insights 
